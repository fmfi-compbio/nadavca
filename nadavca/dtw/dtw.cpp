...
vector<int> RefineAlignment(const vector<double> &signal,
  ...
  vector<MaxNode> prefix_likelihoods(reference.size() + 1),
      suffix_likelihoods(reference.size() + 1);

  prefix_likelihoods[0] = MaxNode(band_starts[0], band_ends[0]);
  for (unsigned i = 0; i < reference.size(); i++) {
    MaxNode predecessor = prefix_likelihoods[i];
    auto distribution = kmer_model.GetDistribution(&sequence, i);
    prefix_likelihoods[i + 1] =
        MaxNode(band_starts[i + 1], band_ends[i + 1], predecessor, distribution,
                signal, min_event_length, false);
  }

  suffix_likelihoods[reference.size()] =
      MaxNode(band_starts[reference.size()], band_ends[reference.size()]);
  for (unsigned i = reference.size(); i > 0; i--) {
    MaxNode predecessor = suffix_likelihoods[i];
    auto distribution = kmer_model.GetDistribution(&sequence, i - 1);
    suffix_likelihoods[i - 1] =
        MaxNode(band_starts[i - 1], band_ends[i - 1], predecessor, distribution,
                signal, min_event_length, true);
  ...
  for (unsigned i = 0; i <= reference.size(); i++) {
    best_signal_index[i] =
        MaxNode::MostLikelyIndex(prefix_likelihoods[i], suffix_likelihoods[i]);
  }
